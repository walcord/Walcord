...
export default function CampaignCard({ campaign }) {
  // Generamos la fecha automática por si te olvidas de rellenar la manual
  const date = new Date(campaign.created_at);
  const fallbackDate = `${date.toLocaleString('en-US', { month: 'short' })}. ${date.getFullYear().toString().slice(2)}'`.toUpperCase();
  
  // Usará tu fecha manual, pero si está vacía, usará la automática
  ...
  return (
    ...
    >
      {/* Imagen Base con Micro-Zoom (Solo en PC con md:group-hover) */}
      <img 
      ...
      {/* Overlay oscuro. 
          MÓVIL: opacity-100 (Siempre visible).
          ORDENADOR: md:opacity-0 -> md:group-hover:opacity-100 (Aparece al hover). 
      */}
      ...
      >
        {/* Fecha top-left (Con block para evitar errores en PC) */}
        <span className="block text-[10px] md:text-xs tracking-[0.25em] font-light uppercase">
          {finalDate}
        </span>
        
        {/* Título y Sujeto (Animación de subida solo en PC) */}
